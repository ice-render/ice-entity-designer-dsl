 ...
 * 1. **铺满容器**：把 canvas 的尺寸对齐容器（示例页是 `.preview`）。老示例页把 canvas 写死成
 *    1200×820 再放到更小的可视区里，`fitViewport()` 是按 canvas 尺寸算的 → 右半边被切掉，
 *    用户看到的就是「图跑到屏幕外」。
 * 2. **滚轮锚点缩放**：`ICE.zoomAt(x, y, factor, min, max)`（引擎内部反解平移并钳制 scale）。
 * 3. **拖拽平移**：空白处左键或任意位置中键拖拽 → `ICE.setViewport`，并把光标切成抓取态。
 *
 * 用法（渲染之后调用，因为「重新渲染」会换掉 canvas 元素）：
 * ```js
 * const result = ICEDSL.renderDsl('canvas', dsl);
 * const view = IedCanvasViewport.install({ ice: result.ice, canvas: document.getElementById('canvas'), designer: result.designer });
 * view.fitViewport();
 * ```
