 ...
 * 示例页冒烟（目录驱动，不写死清单）。
 *
 * 判据：① 无 console error / pageerror；② 无 4xx（示例页会去取 `../dist`、`../node_modules` 下的产物）；
 * ③ 画布**真有落墨** —— 只看"canvas 元素存在"会漏掉"页面崩了、画布空白"这类问题
 * （`entity-editor-dsl.html` 就曾经是这样：canvas 在、但整页报错）。
 *
 * ⚠️ 画布背景是透明的（底色由页面 CSS 给），统计时必须排除 alpha≈0 的像素，否则"非白像素占比"恒为 1。
 */
